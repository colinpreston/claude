...
| Delivery Manager | Coordinates the other desks, tracks what's moving and what's stuck, turns that into action (see Step 7 — not optional) |
...
## Step 6 — Keep every log from rotting: three zones, not one growing pile
...
**Query routing:** a role picking up work reads the Project Brain, the Research Repository's Active Notes and Digest (not its Archive), and its own (plus any dependency role's) Active Notes and Digest. That's it by default. Nobody reads a full Archive to start a normal session — that would just reintroduce the noise the zones exist to keep out.
